 ...
 *
 *  $Id: ChineseSegMain.java 20109 2017-02-13 22:02:05Z markagreenwood $
 */
...
@CreoleResource(name = "Chinese Segmenter PR", comment = "Segment the Chinese text into words, based on the PAUM learning algorithm.", helpURL = "http://gate.ac.uk/userguide/sec:misc-creole:chineseSeg")
public class ChineseSegMain extends AbstractLanguageAnalyser implements
  ...
  public void execute() throws ExecutionException {
    ...
    try {
      ...
      if(!isTraining) {
        dirSeg = new File(Files.fileFromURL(textFilesURL), ConstantParameters.FILENAME_resultsDir);
        if(!dirSeg.exists())
          dirSeg.mkdir();
      }
      ...
      for(File f : xmlFiles) { //for each file in the directory
        if(!f.isDirectory()) {
          ...
          File outSegFile = new File(dirSeg,f.getName()+".seg.txt");
          if(!isTraining) {
            outFeatureVectors = new BufferedWriter(new OutputStreamWriter(
              new FileOutputStream(new File(wdResults,
                ConstantParameters.FILENAMEOFFeatureVectorData)), "UTF-8"));
            outSegText = new BufferedWriter(new OutputStreamWriter(
              new FileOutputStream(outSegFile), this.textCode));
  ...
  @RunTime
  @CreoleParameter(comment="The URL referring to one directory containing the model files")
  ...
  @RunTime
  @CreoleParameter(comment="The URL referring to the directory containing the Chinese text files")
  ...
  @RunTime
  @CreoleParameter(comment="segment Chinese text or learn a model", defaultValue="SEGMENTING")
  ...
  @RunTime
  @CreoleParameter(comment="The code of the Chinese text files used", defaultValue="UTF-8")
  ...
  @RunTime
  @CreoleParameter(comment="Learning algorithm used for learning model", defaultValue="PAUM")
